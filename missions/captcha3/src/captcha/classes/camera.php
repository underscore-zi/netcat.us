 ...
 */
public function setZoom ($zoom) { $this->zoom = $zoom; }
	...
	public function setRandPosition($model){
		$i = (rand(0,10)>5) ? 1 : -1;
		$this->position->x =   $model->getIw()/2+ $i*$model->getIw()/2 + rand(-20, 20);
		$this->position->y = - 80 + rand(-20, 20);
		$this->position->z =   120 + rand(-20, 20);
	...
	public function makeProjection($model){
		...
		//     –асчет коэффициентов матрицы параллельного проецировани€
		// переход от 3D-пространства к 2D-плоскости осуществл€етс€ в виде
		// двух линейных сверток.
		//     “акой подход оказываетс€ наиболее быстрым и соответствует алгоритмам,
		// используемым в таких библиотеках как OpenGL и DirectX.
		...
		// ќпераци€ линейной свертки.
		// ѕереход от 3D-пространства(<X, Y, Z>) к 2D-плоскости(<X, Y>).
